...
class Element:
    ...
    def add_distributed_load(self, q):
        ...
        self.nodes[0].add_load(global_element_load[:3])
        self.nodes[1].add_load(global_element_load[3:])

    def bending_moments(self, u_global, num_points=10):
        """
        Calculate the bending moments along the element.

        Parameters:
        - u_global (numpy.ndarray): Global displacement vector.
        - num_points (int): Number of points to evaluate the bending moments.

        Returns:
        - M (numpy.ndarray): Array of bending moments at the specified points.
        """
        l = self.L
        q = self.q[1]  # Only the vertical load contributes to bending
        EI = self.EI
